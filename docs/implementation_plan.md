...
- **로컬 AI:** Ollama (qwen2.5vl:7b - Vision 지원 모델).
- **지원 언어:** 한국어, 영어.
- **데이터 소스:** 이미지, 텍스트, 마크다운, URL.
- **접근성:** 내부 네트워크 (다른 기기에서도 접속 가능).
- **주요 기능:** 검색, 채팅, 업무 히스토리, 대시보드, 성과 평가.
...
- **백엔드:** Python 3.11+ (FastAPI, Gunicorn).
- **AI/LLM:** Ollama (qwen2.5vl:7b - Vision 지원).
- **RAG 엔진:** SimpleRAGEngine (인메모리, 기본값), ChromaDB (선택적).
...
- [x] LLM 서비스 (핸들러 패턴 리팩토링 완료)
  - HelpHandler: 도움말 제공
  - TreeHandler: 폴더 구조 표시
  - FileListHandler: 파일 목록 조회
  - RAGHandler: 문서 검색 폴백
- [x] RAG 검색 엔진 (SimpleRAGEngine)
- [x] 파일 수집 및 인덱싱 (Watchdog)
- [x] 이미지 분석 (Vision 모델)
